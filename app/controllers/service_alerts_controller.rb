  ...
  def support_call
    if params[:user_id] and params[:user_id] != ''
      @user = User.where(:id => params[:user_id])
        if @user.length != 0
          @update = User.where(:id => params[:user_id]).update_all(:support_call => "1")
            if @update == 1
              return render :json => {:success => true, :message => "User was successfully updated"}  
            else
              return render :json => {:success => false, :message => "User was not successfully updated"}  
            end
        else
